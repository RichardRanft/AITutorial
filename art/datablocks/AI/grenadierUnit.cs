...
function GrenadierUnitData::fire(%this, %obj)
{
    ...
    if(%objWeapon $= "" || !isObject(%objWeapon))
    {
        error(" --- Assigned weapon is invalid");
        return;
    }
    if(%objWeapon.ClassName !$= "WeaponImage")
    {
        error(" --- Assigned weapon is not a WeaponImage");
        return;
    }
    %projectile = %objWeapon.projectile;
    if(%projectile $= "" || !isObject(%projectile))
    {
        error(" --- Weapon has no projectile assigned");
        return;
    }
    %velocity = %projectile.muzzleVelocity;
